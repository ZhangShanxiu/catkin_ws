...
private:
	...
	ros::Subscriber pixel_info_sub_;//接收像素坐标
...
public:
	...
	ImageConverter() : it_( nh_ )
	{
		...
		//像素信息通过订阅话题获取
		pixel_info_sub_ = nh_.subscribe( "/mouse_pixel", 1, &ImageConverter::PixelInfoCb, this );
	...
	void imageColorCb( const sensor_msgs::ImageConstPtr &msg )
	{
		...
		float real_z = 0.001 * depthImage.at<u_int16_t>( Pixel.y, Pixel.x );
		float real_x = (Pixel.x - camera_info.K.at( 2 ) ) / camera_info.K.at( 0 ) * real_z;
		float real_y = (Pixel.y - camera_info.K.at( 5 ) ) / camera_info.K.at( 4 ) * real_z;
	...
	void PixelInfoCb(cute_teleop::pixel msg)
	{
		Pixel.x = msg.x;
		Pixel.y = msg.y;
	}
